...
DDPClient.prototype._prepareHandlers = function () {
  ...
  self.socket.on("error", function (_, error) {
  ...
  self.socket.on("close", function (code, reason) {
    self.emit("socket-close", code, reason);
    self._endPendingMethodCalls();
    self._recoverNetworkError();
  });

  self.socket.on("message", function (_, message) {
...
DDPClient.prototype._makeSockJSConnection = function () {
  ...
  fetch(url)
    ...
    .then(function (response) {
      //var body = response.json(); Does not work returns undefined
    ...
    .catch(function (reason) { self._recoverNetworkError(); });  
};
...
DDPClient.prototype._makeWebSocketConnection = function (url) {
  var self = this;
  self.socket = new WebSocketClient(url, null, self.tlsOpts);
...
DDPClient.prototype.on = function (event, callback, thisArg) {
  this.addListener(event, callback, thisArg);
};


DDPClient.prototype.off = function (event, callback) {
  this.removeListener(event, callback);
};


DDPClient.prototype.addListener = function (event, callback, thisArg) {
...
DDPClient.prototype.removeListener = function (event, callback) {
  if (!Array.isArray(this._callbacks[event])) {
    return;
  }
